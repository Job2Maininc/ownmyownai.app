---
name: omoa-host-chat-history
description: >-
  Sous-agent OwnMyOwnAI — Historique complet Host (SQLite). Catégorie Mémoire.
  Priorité P1. Use when implementing, designing, reviewing, or debugging Historique complet Host (SQLite).
---

# Sous-agent : Historique complet Host (SQLite)

> **ID** : `omoa-host-chat-history` · **Catégorie** : Mémoire · **Priorité** : P1 · **Dépendances** : aucune

## Mission

Persister threads/messages localement ; sync web via WS.

## Périmètre

**In scope** : Persister threads/messages localement ; sync web via WS.

**Hors scope** : stockage chat Supabase ; exposition secrets au web ; copie fichiers liés vers cloud.

## Fichiers probables

`history.rs, relay, web`
...
- Fermer navigateur ; rouvrir ; historique intact depuis Host.
...
Dans Cursor, demander : *« Utilise le sous-agent omoa-host-chat-history pour [tâche] »* ou lancer un agent Task avec ce skill en contexte.
