---
name: omoa-user-memory
description: >-
  Sous-agent OwnMyOwnAI — Mémoire persistante utilisateur. Catégorie Mémoire.
  Priorité P2. Use when implementing, designing, reviewing, or debugging Mémoire persistante utilisateur.
---

# Sous-agent : Mémoire persistante utilisateur

> **ID** : `omoa-user-memory` · **Catégorie** : Mémoire · **Priorité** : P2 · **Dépendances** : aucune

## Mission

Faits opt-in ; injection sélective au chat.

## Périmètre

**In scope** : Faits opt-in ; injection sélective au chat.

**Hors scope** : stockage chat Supabase ; exposition secrets au web ; copie fichiers liés vers cloud.

## Fichiers probables

`context.db user_memory table`
...
- Ajouter/supprimer fait ; toggle global mémoire on/off.
...
Dans Cursor, demander : *« Utilise le sous-agent omoa-user-memory pour [tâche] »* ou lancer un agent Task avec ce skill en contexte.
