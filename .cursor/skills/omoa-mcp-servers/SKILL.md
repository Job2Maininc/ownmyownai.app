---
name: omoa-mcp-servers
description: >-
  Sous-agent OwnMyOwnAI — MCP servers côté Host. Catégorie Codex.
  Priorité P2. Use when implementing, designing, reviewing, or debugging MCP servers côté Host.
---

# Sous-agent : MCP servers côté Host

> **ID** : `omoa-mcp-servers` · **Catégorie** : Codex · **Priorité** : P2 · **Dépendances** : omoa-local-tools

## Mission

Configurer MCP dans Host ; proxy vers agent.

## Périmètre

**In scope** : Configurer MCP dans Host ; proxy vers agent.
...
- Un MCP (ex. filesystem) utilisable depuis le chat web via relay.
...
Dans Cursor, demander : *« Utilise le sous-agent omoa-mcp-servers pour [tâche] »* ou lancer un agent Task avec ce skill en contexte.
