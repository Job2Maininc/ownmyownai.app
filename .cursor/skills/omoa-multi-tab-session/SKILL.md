---
name: omoa-multi-tab-session
description: >-
  Sous-agent OwnMyOwnAI — Session partagée multi-onglets. Catégorie UX Web.
  Priorité P1. Use when implementing, designing, reviewing, or debugging Session partagée multi-onglets.
---

# Sous-agent : Session partagée multi-onglets

> **ID** : `omoa-multi-tab-session` · **Catégorie** : UX Web · **Priorité** : P1 · **Dépendances** : aucune

## Mission

Un seul onglet actif chat ; autres en lecture ou file d'attente.

## Périmètre

**In scope** : Un seul onglet actif chat ; autres en lecture ou file d'attente.

**Hors scope** : stockage chat Supabase ; exposition secrets au web ; copie fichiers liés vers cloud.

## Fichiers probables

`relay-client, Web Locks API`
...
- 2 onglets : message clair, pas de réponses perdues.
...
Dans Cursor, demander : *« Utilise le sous-agent omoa-multi-tab-session pour [tâche] »* ou lancer un agent Task avec ce skill en contexte.
