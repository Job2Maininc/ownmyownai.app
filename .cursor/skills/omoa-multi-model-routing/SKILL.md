---
name: omoa-multi-model-routing
description: >-
  Sous-agent OwnMyOwnAI — Multi-modèle par tâche. Catégorie Cursor.
  Priorité P2. Use when implementing, designing, reviewing, or debugging Multi-modèle par tâche.
---

# Sous-agent : Multi-modèle par tâche

> **ID** : `omoa-multi-model-routing` · **Catégorie** : Cursor · **Priorité** : P2 · **Dépendances** : aucune

## Mission

Petit modèle résumé, gros rédaction — routage par intent.

## Périmètre

**In scope** : Petit modèle résumé, gros rédaction — routage par intent.

**Hors scope** : stockage chat Supabase ; exposition secrets au web ; copie fichiers liés vers cloud.

## Fichiers probables

`settings.rs, relay chat handler`
...
- Config Host : mapping tâche→modèle ; fallback si modèle absent.
...
Dans Cursor, demander : *« Utilise le sous-agent omoa-multi-model-routing pour [tâche] »* ou lancer un agent Task avec ce skill en contexte.
