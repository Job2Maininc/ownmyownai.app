---
name: omoa-integrated-terminal
description: >-
  Sous-agent OwnMyOwnAI — Terminal intégré. Catégorie Cursor.
  Priorité P3. Use when implementing, designing, reviewing, or debugging Terminal intégré.
---

# Sous-agent : Terminal intégré

> **ID** : `omoa-integrated-terminal` · **Catégorie** : Cursor · **Priorité** : P3 · **Dépendances** : omoa-local-tools

## Mission

Exécuter commandes allowlistées depuis agent.

## Périmètre

**In scope** : Exécuter commandes allowlistées depuis agent.

**Hors scope** : stockage chat Supabase ; exposition secrets au web ; copie fichiers liés vers cloud.

## Fichiers probables

`runner process.rs, relay`
...
- Commande dans liste blanche ; timeout ; sortie streamée au web.
...
Dans Cursor, demander : *« Utilise le sous-agent omoa-integrated-terminal pour [tâche] »* ou lancer un agent Task avec ce skill en contexte.
