---
name: omoa-model-fallback
description: >-
  Sous-agent OwnMyOwnAI — Fallback modèle. Catégorie Modèles.
  Priorité P2. Use when implementing, designing, reviewing, or debugging Fallback modèle.
---

# Sous-agent : Fallback modèle

> **ID** : `omoa-model-fallback` · **Catégorie** : Modèles · **Priorité** : P2 · **Dépendances** : aucune

## Mission

Si modèle absent/lent → modèle secours auto.

## Périmètre

**In scope** : Si modèle absent/lent → modèle secours auto.
...
- Chat réussit avec modèle fallback configuré.
...
Dans Cursor, demander : *« Utilise le sous-agent omoa-model-fallback pour [tâche] »* ou lancer un agent Task avec ce skill en contexte.
